...
class VizEncoderDecoder(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(VizEncoderDecoder, self).__init__(init_cfg)
        self.ppt = pretrained
        self.first = True
    ...
    def encode_decode(self, img, img_metas):
        ...
        out, cls_logits = self.decode_head.forward(x) #, img_metas, self.test_cfg)
        if len(out.shape)==3:
            out = out.unsqueeze(0)
        out = resize(
            input=out,
    ...
    def whole_inference(self, img, img_meta, rescale):
        ...
        seg_logit, cls_score = self.encode_decode(img, img_meta)
        if rescale:
    ...
    def inference(self, img, img_meta, rescale):
        ...
        if self.test_cfg.mode == 'slide':
            ...
            seg_logit, cls_score = self.whole_inference(img, img_meta, rescale)
